...
(function(sm, $) {
	sm.validate = {
		...
		options  : {
			 ...
			 *      
			 *      自定义验证,插件将自动将当前dom对象传递给函数
			 *      vfunction="validateFiledFunction"
			 * */
		...
		detailValidate : function($o) {
			...
			if(!('text' == _type || 'hidden' == _type || 'password' == _type || 'textarea' == _type || 'checkbox' == _type || 'file' == _type || $o.is('select')))
				return true;
			...
			var vfunction=$o.attr("vfunction");
			if(bool && vfunction){
				
				var validateResult=eval(vfunction+"($o)");
				if(validateResult){
					sm.validate.highlight($o, validateResult);
					bool = false;
				}
				else{
					sm.validate.unhighlight($o);
